import ModbusAbstractRequest, {
  ModbusAbstractRequestFromBuffer
} from './abstract-request';
import ModbusTCPRequest from './tcp-request';

const debug = require('debug')('modbus-server-request-handler');

export default class ModbusServerRequestHandler<
  FB extends ModbusAbstractRequestFromBuffer<any>
> {
  ...
  handle(data: Buffer) {
    ...
    do {
      ...
      if (request instanceof ModbusTCPRequest && request.corrupted) {
        const corruptDataDump = this._buffer
          .slice(0, request.byteCount)
          .toString('hex');
