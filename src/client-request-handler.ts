 ...
 * @abstract
 */
export default abstract class MBClientRequestHandler<
...
> {
  ...
  constructor(socket: S, timeout: number) {
    if (new.target === MBClientRequestHandler) {
      throw new TypeError(
        'Cannot instantiate ModbusClientRequestHandler directly.'
      );
    }
    this._socket = socket;
  ...
  public abstract register<R extends Req, B extends ModbusRequestBody>(
    requestBody: B
  ): RegisterRequestReturnType<CastRequestBody<R, B>>;
   ...
   */
  public registerRequest<R extends Req>(
    request: R
  ): RegisterRequestReturnType<R> {
  ...
  public handle(response: ModbusAbstractResponse) {
    ...
    if (
      response.body.isException === false &&
      response.body.fc !== request.body.fc
    ) {
  ...
  protected _flush() {
    ...
    const payload =
      this._currentRequest && this._currentRequest.createPayload();
...
export type RegisterRequestReturnType<
  Req extends ModbusAbstractRequest
> = PromiseUserRequest<Req>;
