...
export default class ModbusServerResponseHandler<
  FR extends ModbusAbstractResponseFromRequest
> {
  ...
  private _handleReadCoil(
    ...
    if (!isReadCoilsRequestBody(request.body)) {
      throw new Error(
        `InvalidRequestClass - Expected ReadCoilsRequestBody but received ${request.body.name}`
      );
    }
    ...
    const responseBody = ReadCoilsResponseBody.fromRequest(
      request.body,
      this._server.coils
    );
    const response = this._fromRequest(request, responseBody);
  ...
  private _handleDiscreteInput(
    ...
    if (!isReadDiscreteInputsRequestBody(request.body)) {
      throw new Error(
        `InvalidRequestClass - Expected ReadDiscreteInputsRequestBody but received ${request.body.name}`
      );
    }

    if (!this._server.discrete) {
      debug(
        'no discrete input buffer on server, trying readDiscreteInputs handler'
      );
    ...
    const responseBody = ReadDiscreteInputsResponseBody.fromRequest(
      request.body,
      this._server.discrete
    );
    const response = this._fromRequest(request, responseBody);
    const payload = response.createPayload();
    cb(payload);
  ...
  private _handleReadHoldingRegisters(
    ...
    if (!isReadHoldingRegistersRequestBody(request.body)) {
      throw new Error(
        `InvalidRequestClass - Expected ReadHoldingRegistersRequestBody but received ${request.body.name}`
      );
    }

    if (!this._server.holding) {
      debug(
        'no holding register buffer on server, trying readHoldingRegisters handler'
      );
    ...
    const responseBody = ReadHoldingRegistersResponseBody.fromRequest(
      request.body,
      this._server.holding
    );
    const response = this._fromRequest(request, responseBody);
    const payload = response.createPayload();
    cb(payload);
  ...
  private _handleReadInputRegisters(
    ...
    if (!isReadInputRegistersRequestBody(request.body)) {
      throw new Error(
        `InvalidRequestClass - Expected ReadInputRegistersRequestBody but received ${request.body.name}`
      );
    }

    if (!this._server.input) {
      debug(
        'no input register buffer on server, trying readInputRegisters handler'
      );
    ...
    const responseBody = ReadInputRegistersResponseBody.fromRequest(
      request.body,
      this._server.input
    );
    const response = this._fromRequest(request, responseBody);
  ...
  private _handleWriteSingleCoil(
    ...
    if (!isWriteSingleCoilRequestBody(request.body)) {
      throw new Error(
        `InvalidRequestClass - Expected WriteSingleCoilRequestBody but received ${request.body.name}`
      );
    }
  ...
  private _handleWriteSingleHoldingRegister(
    ...
    if (!isWriteSingleRegisterRequestBody(request.body)) {
      throw new Error(
        `InvalidRequestClass - Expected WriteSingleRegisterRequestBody but received ${request.body.name}`
      );
    }
    ...
    const responseBody = WriteSingleRegisterResponseBody.fromRequest(
      request.body
    );
    ...
    if (responseBody.address * 2 > this._server.holding.length) {
      ...
      this._server.holding.writeUInt16BE(
        responseBody.value,
        responseBody.address * 2
      );
    }
  ...
  private _handleWriteMultipleCoils(
    ...
    if (!isWriteMultipleCoilsRequestBody(request.body)) {
      throw new Error(
        `InvalidRequestClass - Expected WriteMultipleCoilsRequestBody but received ${request.body.name}`
      );
    }
    ...
    const responseBody = WriteMultipleCoilsResponseBody.fromRequest(
      request.body
    );
  ...
  private _handleWriteMultipleHoldingRegisters(
    ...
    if (!isWriteMultipleRegistersRequestBody(request.body)) {
      throw new Error(
        `InvalidRequestClass - Expected WriteMultipleRegistersRequestBody but received ${request.body.name}`
      );
    }

    if (!this._server.holding) {
      debug(
        'no register buffer on server, trying writeMultipleRegisters handler'
      );
    ...
    const responseBody = WriteMultipleRegistersResponseBody.fromRequest(
      request.body
    );

    if (
      request.body.address * 2 + request.body.values.length >
      this._server.holding.length
    ) {
      debug('illegal data address');
      /* illegal data address */
      const responseBody = new ExceptionResponseBody(request.body.fc, 0x02);
